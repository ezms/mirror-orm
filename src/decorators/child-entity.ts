...
export const ChildEntity =
    ...
    <T extends new (...args: Array<any>) => any>(
        ...
        const parentClass = Object.getPrototypeOf(value) as new (
            ...args: Array<any>
        ) => any;
        const parentMeta = registry.getEntity(parentClass.name);
        if (!parentMeta)
            throw new Error(
                `@ChildEntity: parent "${parentClass.name}" has no @Entity registration.`,
            );
        if (!parentMeta.discriminatorColumn)
            throw new Error(
                `@ChildEntity: parent "${parentClass.name}" has no discriminatorColumn.`,
            );

        // Parent columns + child-only columns (child overrides if same propertyKey)
        const parentCols = parentMeta.columns.filter(
            (p) => !childColumns.some((c) => c.propertyKey === p.propertyKey),
        );
        ...
        registry.registerEntity(className, {
            ...
            hooks: {
                beforeInsert:
                    rawHooks.beforeInsert ??
                    parentMeta.hooks?.beforeInsert ??
                    [],
                beforeUpdate:
                    rawHooks.beforeUpdate ??
                    parentMeta.hooks?.beforeUpdate ??
                    [],
                afterLoad:
                    rawHooks.afterLoad ?? parentMeta.hooks?.afterLoad ?? [],
            },
