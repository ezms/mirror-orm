...
export class SqlAssembler<T> {
    ...
    public buildFind(options: IFindOptions<T>): FindPlan {
        ...
        > = {
            'many-to-one': (relation, relatedState) =>
                mtoRelations.push(this.buildMtoInfo(relation, relatedState)),
            'one-to-many': (relation, relatedState) =>
                otmRelations.push({ relation, relatedState }),
            'many-to-many': (relation, relatedState) =>
                mtmRelations.push({ relation, relatedState }),
            'one-to-one': (relation, relatedState) => {
                const isOwner = this.state.metadata.columns.some(
                    (c) => c.databaseName === relation.foreignKey,
                );
                if (isOwner)
                    mtoRelations.push(
                        this.buildMtoInfo(relation, relatedState),
                    );
        ...
        for (const relName of options.relations ?? []) {
            ...
            classifyRelation[relation.type](
                relation,
                this.state.getRelatedState(relation),
            );
        }
        ...
        const baseSelect =
            options.select && options.select.length > 0
                ? options.select
                      ...
                      .map((c) =>
                          qualified
                              ? `${this.state.quotedTableName}.${c.quotedDatabaseName}`
                              : c.quotedDatabaseName,
                      )
                      .join(', ')
                : qualified
                  ? this.state.qualifiedSelectClause
                  : this.state.selectClause;
        let selectPart = baseSelect;

        if (mtoRelations.length > 0) {
            const joinCols = mtoRelations
                .flatMap(({ relatedState, prefix }) =>
                    [...relatedState.columnMap.values()].map(
                        (c) =>
                            `${relatedState.quotedTableName}.${c.quotedDatabaseName} AS "${prefix}${c.databaseName}"`,
                    ),
                )
                .join(', ');
            selectPart += `, ${joinCols}`;
        ...
        if (options.orderBy) {
            const orderClauses = Object.entries(options.orderBy)
                .map(([key, direction]) => {
                    ...
                    return column
                        ? `${column.quotedDatabaseName} ${direction}`
                        : null;
                })
        ...
        sql += this.state.buildLimitOffset(
            hasOrderBy,
            options.limit,
            options.offset,
        );
    ...
    public buildInsert(
        ...
        const columns = this.state.metadata.columns.filter(
            (c) =>
                (!c.primary || !isIdentity) &&
                effectiveRecord[c.propertyKey] !== undefined,
        );
        const names = columns.map(
            (c) => this.state.columnMap.get(c.propertyKey)!.quotedDatabaseName,
        );
        const params = columns.map((c) => effectiveRecord[c.propertyKey]);
        const placeholders = params.map((_, i) =>
            this.state.placeholder(i + 1),
        );
        return {
    ...
    public buildBulkInsert(
        ...
        const columns = this.state.metadata.columns.filter(
            (c) =>
                (!c.primary || !isIdentity) &&
                effective.some((r) => r[c.propertyKey] !== undefined),
        );
        const names = columns.map(
            (c) => this.state.columnMap.get(c.propertyKey)!.quotedDatabaseName,
        );
    ...
    public buildUpdateById(
        ...
        const columns =
            dirtyColumns ??
            this.state.metadata.columns.filter(
                (c) =>
                    !c.primary &&
                    !c.version &&
                    record[c.propertyKey] !== undefined,
            );
        const setClauses = columns.map(
            (c, i) =>
                `${this.state.columnMap.get(c.propertyKey)!.quotedDatabaseName} = ${this.state.placeholder(i + 1)}`,
        );
        const params: Array<unknown> = [
            ...columns.map((c) => record[c.propertyKey]),
        ];

        if (versionInfo) {
            setClauses.push(
                `${versionInfo.column.quotedDatabaseName} = ${this.state.placeholder(params.length + 1)}`,
            );
    ...
    public buildUpdate(
        ...
        const params: Array<unknown> = [];
        const columns = this.state.metadata.columns.filter(
            (c) => !c.primary && record[c.propertyKey] !== undefined,
        );
        if (columns.length === 0)
            throw new QueryError(
                'UPDATE',
                new Error('No updatable columns provided'),
            );
        ...
        const whereSql = this.appendDiscriminatorFilter(
    ...
    public buildDelete(where: IFindOptions<T>['where']): {
        sql: string;
        params: Array<unknown>;
    } {
        const params: Array<unknown> = [];
    ...
    public buildUpsert(
        ...
        const insertCols = this.state.metadata.columns.filter(
            (c) =>
                (!c.primary || !isIdentity) &&
                effectiveRecord[c.propertyKey] !== undefined,
        );
        const names = insertCols.map(
            (c) => this.state.columnMap.get(c.propertyKey)!.quotedDatabaseName,
        );
        ...
        const setClauses = updateCols.map(
            (c) => `${c.quotedDatabaseName} = EXCLUDED.${c.quotedDatabaseName}`,
        );
    ...
    public buildRemoveMany(
        pk: IColumnMetadata & { quotedDatabaseName: string },
        ids: Array<unknown>,
    ): { sql: string; params: Array<unknown> } {
        const params: Array<unknown> = [];
        const inClause = this.state.buildArrayInClause(
            pk.quotedDatabaseName,
            ids,
            params,
        );
        return {
            sql: `DELETE FROM ${this.state.quotedTableName} WHERE ${inClause}`,
            params,
        };
    }

    private withDiscriminator(
        record: Record<string, unknown>,
    ): Record<string, unknown> {
        if (
            !this.state.metadata.discriminatorValue ||
            !this.state.metadata.discriminatorColumn
        )
            return record;
        const discCol = this.state.metadata.columns.find(
            (c) => c.databaseName === this.state.metadata.discriminatorColumn,
        );
        if (!discCol) return record;
        return {
            ...record,
            [discCol.propertyKey]: this.state.metadata.discriminatorValue,
        };
    }
    ...
    private buildWhereGroup(
        ...
        for (const [key, value] of Object.entries(condition)) {
            ...
            if (isOperator(value)) {
                if (
                    value.requiresJsonSupport &&
                    !this.state.supportsJsonOperators
                ) {
                    throw new Error(
                        `JSON operators are only supported on PostgreSQL. The current dialect does not support JSON operators.`,
                    );
                }
                const { sql, params: opParams } = value.buildClause(
                    column.quotedDatabaseName,
                    params.length + 1,
                ...
                params.push(value);
                clauses.push(
                    `${column.quotedDatabaseName} = ${this.state.placeholder(params.length)}`,
                );
            }
        }
        return clauses;
