...
import { Column, CreatedAt, DeletedAt, Entity, PrimaryColumn, UpdatedAt } from '../index';
...
function makeRunner() {
    ...
    const defaultRow = [{ id: 'gen-id', title: 'x', created_at: new Date(), updated_at: new Date(), deleted_at: null }];
    const runner = {
...
describe('@CreatedAt and @UpdatedAt', () => {
    it('sets createdAt and updatedAt on INSERT', async () => {
        ...
        const insert = runner.calls.find(c => c.sql.includes('ts_posts') && c.sql.includes('INSERT'));
    ...
    it('does not set createdAt on UPDATE', async () => {
        ...
        const post = Object.assign(new TsPost(), { id: 'existing-id', title: 'Hello' });
        await repo.save(post);

        const update = runner.calls.find(c => c.sql.includes('ts_posts') && c.sql.includes('UPDATE'));
...
describe('@DeletedAt — soft delete', () => {
    it('emits UPDATE with deleted_at instead of DELETE', async () => {
        ...
        const article = Object.assign(new TsArticle(), { id: 'article-1', title: 'Gone' });
        await repo.remove(article);

        const sqls = runner.calls.map(c => c.sql);
        expect(sqls.every(s => !s.includes('DELETE'))).toBe(true);
        const softDelete = runner.calls.find(c => c.sql.includes('ts_articles') && c.sql.includes('UPDATE'));
    ...
    it('softRestore sets deleted_at to null', async () => {
        ...
        const article = Object.assign(new TsArticle(), { id: 'article-1', title: 'Back', deletedAt: new Date() });
        await repo.softRestore(article);

        const update = runner.calls.find(c => c.sql.includes('ts_articles') && c.sql.includes('UPDATE'));
        expect(update).toBeDefined();
